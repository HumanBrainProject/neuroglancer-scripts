...
from neuroglancer_scripts.sharded_base import (
    ShardSpec, CMCReadWrite, ShardedScaleBase, ReadableMiniShardCMC, ShardedAccessorBase, ShardVolumeSpec
)
...
class HttpShard(CMCReadWrite):
    ...
    def __init__(self, base_url, session: requests.Session, shard_key: np.uint64, shard_spec:ShardSpec):
        super().__init__(shard_spec)
        
        self.shard_key_str = hex(shard_key)[2:].rjust(math.ceil(self.shard_spec.shard_bits / 4), "0")
        ...
        if self.file_exists(f"{self.shard_key_str}.index") and self.file_exists(f"{self.shard_key_str}.data"):
            self.is_legacy = True
        else:
    ...
    def read_bytes(self, offset: int, length: int) -> bytes:
        ...
        assert len(content) == length, f"Getting {file_url} error. Expecting {length} bytes (Range: {range_value}), but got {len(content)}."
        return content
...
class HttpShardedScale(ShardedScaleBase):
    ...
    def __init__(self, base_url: str, session: requests.Session, key: str, shard_spec: ShardSpec, shard_volume_spec: ShardVolumeSpec):
    ...
    def get_shard(self, shard_key: np.uint64) -> CMCReadWrite:
        if shard_key not in self.shard_dict:
            self.shard_dict[shard_key] = HttpShard(f"{self.base_url}{self.key}/", self._session, shard_key, self.shard_spec)
        return self.shard_dict[shard_key]
    

class ShardedHttpAccessor(neuroglancer_scripts.http_accessor.HttpAccessor, ShardedAccessorBase):
    def __init__(self, base_url):
        neuroglancer_scripts.http_accessor.HttpAccessor.__init__(self, base_url)
    ...
    def fetch_chunk(self, key, chunk_coords):
        if key not in self.shard_dict:
            ...
            self.shard_dict[key] = HttpShardedScale(self.base_url, self._session, key, shard_spec, shard_volume_spec)
